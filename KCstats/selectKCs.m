...
mInputArg = varargin{1};

if isempty(strmatch('twoPhoton',class(mInputArg)))
  error('Input should be of class ''twoPhoton''')
end

disp('setting up image array')
if length(mInputArg)==1
  frames=[];
  inputim = double(mean(mInputArg.imageStack,3));
elseif length(mInputArg)>1
  frames=zeros([size(mInputArg(1).imageStack,1),...
                size(mInputArg(1).imageStack,2),...
                length(mInputArg)]);
  tmp=frames;
  if isfield(mInputArg(1).info,'muStack')
    for i=1:length(mInputArg)
      frames(:,:,i)=mInputArg(i).info.muStack;
    end
  else
    for i=1:length(mInputArg)
      frames(:,:,i)=mean(mInputArg(i).imageStack,3);
    end
  end
...
prop=properties(mInputArg(1));
if strmatch('KCmasks',prop)
    roimasks=mInputArg(1).KCmasks;
    if ~isempty(roimasks)
    u=unique(roimasks(:));
    u=u(end);
...
for i=1:length(varargin{1})
    if isempty(strmatch('KCmasks',properties(mInputArg(i))))
        addprop(mInputArg(i),'KCmasks');
    end
    mInputArg(i).KCmasks=roimasks;
end
varargout{1}=mInputArg;
varargout{2}=frames;
...
%            set(hImgEditAxes, 'Ylim', ylimit, 'Xlim', xlimit);
          ...
          num = str2double(get(hCellCount, 'string'));
          lastmask(find(roimasks == num)) = 1;
          prevmasks(find(roimasks > 0 & roimasks < num)) = 1;
