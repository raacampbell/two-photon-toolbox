function [level,bw] = thresh_tool(im,defaultLevel,nonInteractive) %mainfunction
%THRESH_TOOL  Interactively select intensity level for image thresholding
%
% NOTE:
% This function was downloaded from Matlab Central and modified for
% use with the imaging data. Also some corrections to deal with
% warning messages. Tested on Matlab 2008a to 2013a. 
%
...
%%NOTE
% if the third argument is 1, then an output is produced without
% use input
...
if nargin>1 %user specified DEFAULTLEVEL
  dataType = class(im);
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
% Set up axes
%h_ax1: Top left - input image
h_ax1 = axes('unit','norm','pos',[0.05 0.35 0.4 0.60]);
rgb = mat2im(im,full_map);
...
v=version; 
if str2num(v(1:3))>=7 
  linkaxes([h_ax1 h_ax2 h_ax3]) %link top axes (pan & zoom)
  linkaxes([h_hist h_cbar],'x') %link bottom axes (X only - pan & zoom)
end
...
if nargin>1 %user specified default level
  ...
  %I find that graythresh tends to underestimate a little for our images:
  norm_level=norm_level*0.8;
...
axes(h_ax2)
hold on
subimage(bw.*im), axis off, axis ij
title('ROI')
update_plot;
...
function bw=update_plot %subfunction
im = getappdata(gcf,'im');
h_lev = getappdata(gcf,'h_lev');
my_level = mean(get(h_lev,'xdata'));
h_ax2 = getappdata(gcf,'h_ax2');
h_ax3 = getappdata(gcf,'h_ax3');
h_im2 = findobj(h_ax2,'type','image');

warning off
bw = im2bw(im,my_level);%segmented image

%Smooth the border
st=strel('disk',5);bw=imclose(bw,st);
st=strel('disk',3);bw=imdilate(bw,st);
%bw=imfill(bw,'holes');
warning on

%make pixels outside ROI a different color using an rgb image
rgb_bw = repmat(double(bw).*im,[1 1 3]);
rgb_bw(:,:,1)=rgb_bw(:,:,1)*0.05;
rgb_bw(:,:,3)=rgb_bw(:,:,3)*0.05;

rgb_im = repmat(double(im),[1 1 3]);
rgb_im(:,:,2) = rgb_im(:,:,2).*0.05;
rgb_im(:,:,3) = rgb_im(:,:,3)*0.05;

rgb_version=rgb_im+rgb_bw;
rgb_version=rgb_version/max(rgb_version(:));
rgb_version(:,:,1) = im;
...
cla
%overlay the borders
border=bwboundaries(bw);
%In case there are multiple borders choose the largest
hold on
for i=1:length(border)  
 plot(border{i}(:,2),border{i}(:,1), 'r-')
end
hold off
